...
# REAL IMPORT - ZEVRIC
try:
    from real_server_api_ff import RealFFServerAPI, create_bot_real_server_api
    real_api = RealFFServerAPI(region="IND")
    REAL_API_AVAILABLE = True
    print("✅ Real API loaded - ZEVRIC")
except Exception as e:
    print(f"⚠️ Real API import failed: {e}")
    real_api = None
...
Bots: {', '.join([b['name'] for b in bots_created[:3]])}...
"""
...
async def help_cmd(update, context: ContextTypes.DEFAULT_TYPE):
    try:
        uid = update.effective_user.id
        name = update.effective_user.first_name
        print(f"🆘 /help called by {uid} {name}")
        text = f"""🆘 HELP CENTER - ZEVRIC GLORY STORE

👋 Hey {name}!

❓ Kaise Use Kare:
1. /addbalance - UPI se payment karo
2. Screenshot + UTR bhejo
3. /buycredits se credit kharido
4. /myguilds me guild ID set karo
5. /launch se bot launch karo
...
📞 Support:
Admin: @{SUPPORT}
Kisi ko help chahiye? Message karlo!
"""
        kb = [[InlineKeyboardButton("🏠 Home", callback_data="back_home"), InlineKeyboardButton("📞 Contact Admin", url=f"https://t.me/{SUPPORT}")]]
        await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(kb))
        print(f"✅ /help replied to {uid}")
    except Exception as e:
        print(f"❌ /help error: {e}")
        import traceback
        traceback.print_exc()
        try:
            await update.message.reply_text(f"🆘 Help - Contact @{SUPPORT} for any issue!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📞 Contact Admin", url=f"https://t.me/{SUPPORT}"), InlineKeyboardButton("🏠 Home", callback_data="back_home")]]))
        except:
            pass
...
async def main():
    while True:
        try:
            ...
            # ULTIMATE FIX: Delete webhook via direct HTTP request FIRST
